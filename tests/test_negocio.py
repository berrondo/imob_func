...
def test_criar_negocio(banco):
    j1 = criar_jogador(estrategia=impulsivo, saldo=CEM).set('i', 0)
    p1 = criar_propriedade(preco=30, aluguel=DEZ).set('i', 0)

    n = comprar_ou_alugar(j1, p1, C, banco)
...
def test_impulsivo_compra_se_tem_saldo(preco, comprou, banco):
    j1 = criar_jogador(estrategia=impulsivo, saldo=CEM).set('i', 0)
    p1 = criar_propriedade(preco=preco, aluguel=DEZ).set('i', 0)
...
def test_exigente_compra_se_tem_saldo_e_aluguel_maior_que_50(preco, comprou, banco):
    j1 = criar_jogador(estrategia=exigente, saldo=CEM).set('i', 0)
    p1 = criar_propriedade(preco=preco, aluguel=51).set('i', 0)
...
def test_cauteloso_compra_se_sobrar_pelo_menos_80(preco, comprou, banco):
    j1 = criar_jogador(estrategia=cauteloso, saldo=CEM).set('i', 0)
    p1 = criar_propriedade(preco=preco, aluguel=DEZ).set('i', 0)
...
def test_aleatorio_compra_conforme_random(monkeypatch, random_choice, banco):
    ...
    j1 = criar_jogador(estrategia=aleatorio, saldo=CEM).set('i', 0)
    p1 = criar_propriedade(preco=CEM, aluguel=DEZ).set('i', 0)
